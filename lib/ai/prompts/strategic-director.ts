...
2. **Growth Formula (内容配比)**:
   - 根据行业生成最佳的内容比例(饼图数据)
   - 例如餐饮业: 40% Visual ASMR (视觉诱惑) + 30% Social Proof (评价) + 30% BTS (幕后)
   - 例如服务业: 35% Results (成果) + 35% Expertise (专业) + 30% Community (社区)

3. **30-Day Calendar (智能日历)**:
   - **Day 1 (明天)**: 必须是"Ready-to-Post"级别,包含:
     * 完整Caption (200字,包含Emoji和行动召唤)
     * 10个Hashtags (大中小热度混合,包含本地标签)
     * **高质量英文生图提示词** (用于Sidewalk AI)
   - **Day 2-30**: 仅提供策略标签和简短灵感,格式: "主题 - 创意点子"

4. **Tone**: 专业、鼓励性、直接点出商业价值 (Not just "likes", but "sales")
